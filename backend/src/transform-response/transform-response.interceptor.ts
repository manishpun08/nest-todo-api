...
export class TransformResponseInterceptor implements NestInterceptor {
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<SuccessResponse> {
    ...
    const metadataEntity = this.reflector.get<string>(ENTITY_KEY, context.getHandler());
    const metadataMessage = this.reflector.get<string>(MESSAGE_KEY, context.getHandler());
    const entity = metadataEntity || this.getEntityName(request.url);

    return next.handle().pipe(
      map((response: unknown) => {
        const isPaginated =
          response && typeof response === 'object' && 'items' in response && 'meta' in response;

        const data = isPaginated ? (response as PaginatedResponse).items : response;
        const meta = isPaginated ? (response as PaginatedResponse).meta : undefined;
